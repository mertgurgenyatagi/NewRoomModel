...
- **Files:** `main.tscn` (scene), `main.gd` (collision, material shaders, sun, light toggle), `player.gd` (walker), `assets/room.glb` (the exported room), `floor_tiles.gdshader` and `shaders/` (`plaster` roughcast with bump, `door` striped gloss black, `counter_top` speckled, `facade` windows on the buildings). `main.gd` swaps the flat glTF colours for these shaders by material name.
...
- **Lighting:** plain natural daylight: a neutral white afternoon sun about 40 degrees up from the window side (real sun patch and mullion shadows on the floor), neutral warm-white ambient light, SSAO and SSIL for contact shading. The glass and blinds must not cast shadows or they block the sun. SDFGI was tried twice and dropped: it left the interior almost black. Baked lighting (LightmapGI) is the next step up if it needs more depth.
